    ...
    try:    
        ...
        for i in (data):
            try:
                    ...
                    "Employee_Type":Type,
                    "Employee_Effective_Date":HRDate,
                    "Employee_Hire_Date":JHDate,
                    ...
                    try:
                        if type((i)['wd:Compensation_History']) is dict:
                            try:
                                ...
                                HRDate =  employee_data[count-1]["Employee_Effective_Date"]
                                employee_data[count-1]["Employee_Effective_Date"] = i["wd:Compensation_History"]["wd:Effective_Date_Compensation"]
                            except:
                            ...
                            "Employee_Current_Region":Region,
                            "Employee_Current_Pay_Group":PayG}
                            #print("Dcitionary --->",HRDate,employee_data[count-1]["Employee_Effective_Date"],employee_data[count]["Employee_Effective_Date"])
                            #employee_data[count]['Employee_Effective_Date'] = nHRDate
                            count+=1
                            ...
                            for j in range(len(i['wd:Compensation_History'])):
                                ...
                                "Employee_Current_Pay_Group":PayG}
                                count+=1
                            #employee_data[count-1-len(datelist)]['Employee_Effective_Date'] = datelist[-1]
                            #for j in range(1,len(datelist)):
                            #    employee_data[count-1-len(datelist)+j]['Employee_Effective_Date'] = datelist[j-1]
                            #temp = employee_data[count-counterdates-1]["Employee_Current_Salary"]
                            #for j in range(counterdates,0,-1):
                            #    #employee_data[count-1-j]["Employee_Effective_Date"] =
                            #    back = j+1
                            ...
                            #print("list--->",HRDate,employee_data[count-2]["Employee_Effective_Date"],employee_data[count-1]["Employee_Effective_Date"])
                            #for j in range(len(datelist)):
                            #    employee_data[count-1-j]['Employee_Effective_Date']=datelist[j]
                            #employee_data[count-1-len(datelist)]['Employee_Effective_Date'] = datelist[0]
                            #employee_data[count-len(datelist)]['Employee_Effective_Date'] = datelist[-1]
                            #employee_data[count-1]['Employee_Effective_Date'] = datelist[-1]
                            #for j in range(len(datelist),0,-1):
                            #    employee_data[count-1-j]['Employee_Effective_Date'] = datelist[len(datelist)-1-j]
                            #employee_data[count-1-len(datelist)]['Employee_Effective_Date'] = datelist[-1]
                            #print(datelist)
                            #####for k in range(0,len(datelist)):
                            #####    print(employee_data[count-len(datelist)+k]['Employee_Effective_Date'],employee_data[count-len(datelist)+k]['Employee_Current_Salary'])
                            employee_data[count-len(datelist)]['Employee_Effective_Date'] = datelist[-1]
                            for k in range(1,len(datelist)):
                                employee_data[count-len(datelist)+k]['Employee_Effective_Date'] = datelist[k-1]
                            #print("\n")
                            #for k in range(0,len(datelist)):
                            #    print(employee_data[count-len(datelist)+k]['Employee_Effective_Date'],employee_data[count-len(datelist)+k]['Employee_Current_Salary'],datelist[k-1])
                    except :
                        pass
            except:
            ...
            for i in (data):
                    ...
                    try:
                        if type((i)['wd:Compensation_History']) is dict:
                            ...
                            "Employee_Current_Region":Region,
                            "Employee_Current_Pay_Group":PayG}
                            employee_data[count-1]['Employee_Effective_Date'] = nHRDate
                            count+=1
                        elif type((i)['wd:Compensation_History']) is list:
                                counterdates=0
                                for j in range(len(i['wd:Compensation_History'])):
    ...
    try:    
        ...
        try:
            ...
            for i in (data):
                    ...
                    "Employee_Effective_Date":i["wd:Hire_Date"],
                    ...
                    try:
                        if type((i)['wd:Compensation_History']) is dict:
                            ...
                            "Employee_Status":i["wd:Active_Status"],
                            "Employee_Type":i["wd:Employee_Type"],
                            "Employee_Effective_Date":i["wd:Compensation_History"]["wd:Effective_Date_Compensation"],
                            "Employee_PF_Time":i["wd:Time_Type"]['type'][1],
                            ...
                            count+=1
                                ...
                                for j in range(len(i['wd:Compensation_History'])):
                                    ...
                                    "Employee_Status":i["wd:Active_Status"],
                                    "Employee_Type":i["wd:Employee_Type"],
                                    "Employee_Effective_Date":i["wd:Compensation_History"][j]["wd:Effective_Date_Compensation"],
                                    "Employee_PF_Time":i["wd:Time_Type"]['type'][1],
...
def get_dict_with_condition2(employee_data):
        logger.info("Removing data that doesn't complies with condition")
    #try:
        employees_with_benefits = {}
        count = 0
        for i in employee_data.keys():
            ...
            if count >=1:
                #print(i,employee_data[i])
                #if employee_data[i]["Employee_Effective_Date"] == employees_with_benefits[count-1]["Employee_Effective_Date"] and employee_data[i]["Employee_ID"] == employees_with_benefits[count-1]["Employee_ID"]:
                    if str( employees_with_benefits[count-1]["Employee_Current_Bonus_Percent"]).strip() == "0" or str(employees_with_benefits[count-1]["Employee_Current_Salary"]).strip() == "0":
                        count-=1
            employees_with_benefits[count]=employee_data[i]
            count+=1
    #except:
...
def get_dict_with_condition3(employee_data):
        logger.info("Removing data that doesn't complies with condition")
    #try:
        employees_with_benefits = {}
        count = 0
        for i in employee_data.keys():
            ...
            if count >=1:
                #print(i,employee_data[i])
                if employee_data[i]["Employee_Effective_Date"] == employees_with_benefits[count-1]["Employee_Effective_Date"] and employee_data[i]["Employee_ID"] == employees_with_benefits[count-1]["Employee_ID"]:
                    if float(employee_data[i]["Employee_Current_Salary"]) > float(employees_with_benefits[count-1]["Employee_Current_Salary"]) and  (employee_data[i]["Employee_Current_Currency"] == employees_with_benefits[count-1]["Employee_Current_Currency"]):
                        count-=1
                    else:
                        continue
            employees_with_benefits[count]=employee_data[i]
...
def get_data_before_cutoff(employee_data,cutoffdate):
    ...
    try:
        ...
        for i in employee_data.keys():
            try:
                dateHR =  datetime.strptime(employee_data[i]["Employee_Effective_Date"][0:-6], '%Y-%m-%d')
            except:
                dateHR =  datetime.strptime(employee_data[i]["Employee_Effective_Date"], '%Y-%m-%d')
...
def get_data_before_cutoff(employee_data,cutoffdate):
    ...
    try:
        ...
        for i in employee_data.keys():
            try:
                dateHR =  datetime.strptime(employee_data[i]["Employee_Hire_Date"][0:-6], '%Y-%m-%d')
            except:
                dateHR =  datetime.strptime(employee_data[i]["Employee_Hire_Date"], '%Y-%m-%d')
            if dateHR <= cutoffdate:
                employees_with_benefits[count]=employee_data[i]
                count+=1
    except:
...
def order_dic(dic,rev):
    ...
    #print(dic.keys())
    try:
        list_from_dict = sorted(dic.values(), key=itemgetter("Employee_ID", "Employee_Effective_Date"), reverse=rev)
        out_dic = {}
        for i in range(len(list_from_dict)):
            out_dic[i] = list_from_dict[i]
    except:
        ...
        with open(f"{data_path}\\calculation.txt","w") as f:
            for i in Employee_dic.keys():
                ...
                if  "Fixed Percent" in first_data[5]:
                    print("Bonus")
                    data_to_write+=f"Salary for employee {employee_current_id} : {first_data[6]} {from_currency}\n"
    ...
    #try:
        ...
        for i in pivot.keys():
            ...
            for j in pivot[i].keys():
                ...
                print(CR)
                conversion_rate = Decimal(CR) #round(CR)# end_date)   )
...
def Create_SOAP_Body(ledger_id, period_name, accounting_date, group_id, currency, amount, reversal, account, ent, dep, Journal_name,ledger_name,Ledger_Description,type):
    ...
    if type == 1:
        crdr = "Cr"
    if "OCT" in reversal.upper():
        print(f"\n\n\n{reversal}\n")
        reversal=reversal.replace("Oct","Sep")
        print(f"\n{reversal}\n\n\n\n")
    return f'''               <jour:GlInterface>
               ...
               <jour:Reference9>N</jour:Reference9>
            </jour:GlInterface>\n'''
