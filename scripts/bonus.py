...
def get_bonus():
        ...
        try:
                ...
                url = Create_Workday_Url(Workday_String_Start_date,Workday_String_End_date,"xml")
                if "Error" in url:
                        Push_To_S3(f"{logs_path}\\functions.log","Process4","Log")
                        return f"{url} // "
                ...
                employee_data = get_dict_with_condition2(employee_data)
                employee_data = get_dict_with_condition3(employee_data)
                if type(employee_data) == str:
                        Push_To_S3(f"{logs_path}\\functions.log","Process4","Log")
                        return f"{employee_data}  //"
